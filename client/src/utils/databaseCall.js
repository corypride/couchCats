...
databaseCall.interceptors.request.use(async (config) => {
    try {
      const response = await axios.get(`http://localhost:8080/user/secure`, { headers: headersObj, withCredentials: true });
      console.log("success")
...
//     console.log("WORKED")
//     axios.get(`http://localhost:8080/user/secure`, { headers: headersObj, withCredentials: true })
//     .then(response => {
//       console.log("response from test => ", response);
//       return config;
//     })
